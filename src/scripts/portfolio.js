...
const portfolioContainer = document.getElementById('portforlio');


function createCard (data) {
      const cartDiv = document.createElement('div');
      cartDiv.className = 'card'
      
      //card image
      const cartImage = document.createElement('img');
      cartImage.src = `./src/img/portfolio/${data.type}/1.png`;

      //card animate
      const cardAnimate = document.createElement('div');
      cardAnimate.className = 'card-animate';
      const span = document.createElement('span');
      span.className = 'card-span';
      span.textContent = data.title

      const link = document.createElement('a');
      link.href = `${data.url}#${hash}`;
      link.className = 'card-web';

      const btn = document.createElement('button');
      btn.className = 'card-btn'
      btn.classList.add('lang-bttn')
      btn.textContent = 'Подробнее'

      link.append(btn)

      cardAnimate.append(span)
      cardAnimate.append(link)


      cartDiv.append(cartImage);
      cartDiv.append(cardAnimate)


      portfolioContainer.append(cartDiv);
}
...
btnMobile.addEventListener('click', function(e) {
   ...
   portfolioContainer.innerHTML = '';
   for (let i = 0; i < PORTFOLIO_MOBILE_INFO.length; i++) {
      createCard(PORTFOLIO_MOBILE_INFO[i])
   }
   btnLanguage()
})
