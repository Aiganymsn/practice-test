...
function changeLanguage() {
 ...
 let bttn = document.getElementsByClassName('lng-servicebtn')

   for (let i in bttn) {
      bttn[i].textContent = langArr['servicebtn'][hash]
   }
   ...
   function createServicesCard(data) {
      ...
      itemTitle.textContent = data.title[hash];

      const itemText = document.createElement('h4');
      itemText.textContent = data.subtitle[hash];

      //button
      const serviceLink = document.createElement('a');
      serviceLink.href = `${data.url}#${hash}`;

      const serviceBtn = document.createElement('button');
      serviceBtn.className = 'service-btn';
      serviceBtn.classList.add('lang-bttn');
      serviceBtn.textContent = 'Перейти';

      serviceLink.append(serviceBtn);

      itemCaption.append(itemTitle);
      itemCaption.append(itemText);
      itemCaption.append(serviceLink);

      serviceDiv.append(itemCaption);
      servicesContainer.append(serviceDiv)
   }


   for (let i = 0; i < PROGRAMS_INFO .length; i++) {
      createServicesCard(PROGRAMS_INFO [i])
   }



   function btnLanguage () {
      let btn = document.getElementsByClassName('lang-bttn')
   
      for (let i in btn) {
         btn[i].textContent = langArr['bttn'][hash]
      }
   }
   btnLanguage()
