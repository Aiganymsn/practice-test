...
const servicesInnerPage = document.getElementById('services-inner');
...
const findSeviceChild = SERVICES_INFO.find(item => item.id === id).child

text.textContent = findSeviceChild[0].mainTitle[hash];
function createInnerServicesCard(data) {
   //big container
   const serviceInnerDiv = document.createElement('div');
   serviceInnerDiv.className = 'services-items'
  
   //img
   const serviceInnerImg = document.createElement('img');
   serviceInnerImg.src = data.img;
   serviceInnerDiv.append(serviceInnerImg);

   //item container
   const itemInnerCaption = document.createElement('div');
   itemInnerCaption.className = 'item-caption';

   const itemInnerTitle = document.createElement('h2');
   itemInnerTitle.textContent = data.title[hash];


   const itemInnerText = document.createElement('h4');
   itemInnerText.textContent = data.cost[hash];

   //button
   const serviceInnerLink = document.createElement('a');
   serviceInnerLink.href = data.src + `#${hash}`;

   const serviceInnerBtn = document.createElement('button');
   serviceInnerBtn.className = 'service-btn';
   serviceInnerBtn.id = data.id;
   serviceInnerBtn.textContent = data.button[hash];

   serviceInnerLink.append(serviceInnerBtn)

   itemInnerCaption.append(itemInnerTitle)
   itemInnerCaption.append(itemInnerText)
   itemInnerCaption.append(serviceInnerLink)

   serviceInnerDiv.append(itemInnerCaption);
   servicesInnerPage.append(serviceInnerDiv)
}

// createInnerServicesCard();

for (let i = 0; i < findSeviceChild.length; i++) {
   createInnerServicesCard(findSeviceChild[i])
}
